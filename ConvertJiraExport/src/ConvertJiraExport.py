...
def main(argv):
    ...
    # Basic input file checking: does it exist, is it a csv?
    if not exists(input_filename):
        print(f'File "{input_filename}" does not exist, exiting.')
        exit()
    if Path(input_filename).suffix != '.csv':
        print('Input file must be a CSV file, exiting')
        exit()
    
    # Initialise message sets
    messages = []        # Processing Status messages
    ticket_messages = [] # Column-specific messages
    ...
    # Set output field list (tuple) and order
...
#                   'Flagged')
    ...
    # Create the output fields list from the 'Output Column' of the mapping list
    # above. Note that the 'inp' field is not used and is simply to obtain the
    # correct values from the list
    output_list = list(col_name for inp, col_name in column_mapping)
    
    # Insert additional column header for 'last sprint'
    try:
        idx = output_list.index('Sprints')
        output_list.insert(idx, 'Last Sprint')
    except ValueError: # If 'Sprints' field not found, assume not required
        pass
    ...
    # Check for output file already existing. If it exists, rename it. This could
    # still fail if the destination of the rename exists, so put it in an exception block
    if exists(output_filename):
        overwrite = input(f'Destination file {output_filename} exists, overwrite (y/N)? ')
    ...
    try:
        with open(output_filename, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            for row in enumerate(data[1:]):
                ...
                # Set the field values NOTE not negative tested yet
                try:
                    output_dict['Sprints'] = sprints
                except KeyError:
                    pass
                try:
                    output_dict['Last Sprint'] = last_sprint
                except KeyError:
                    pass
    ...
    # Catch any exceptions 
        ...
        if err.errno == errno.EACCES:
            print(f'Destination file {output_filename} could not be written to. Check it\'s not open elsewhere')
        else:
            print(f'Error opening destination file {output_filename} for writing')
        print('Exiting')
        exit()
